
namespace ITMO.CSCourse.Lab01_Ex05._01
{
	partial class Form1
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.FormCloseButton = new System.Windows.Forms.Button();
			this.SuspendLayout();
			// 
			// FormCloseButton
			// 
			this.FormCloseButton.DialogResult = System.Windows.Forms.DialogResult.Cancel;
			this.FormCloseButton.Location = new System.Drawing.Point(236, 98);
			this.FormCloseButton.Name = "FormCloseButton";
			this.FormCloseButton.Size = new System.Drawing.Size(100, 50);
			this.FormCloseButton.TabIndex = 0;
			this.FormCloseButton.Text = "Close Form";
			this.FormCloseButton.UseVisualStyleBackColor = true;
			this.FormCloseButton.Click += new System.EventHandler(this.button1_Click);
			// 
			// Form1
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.BackColor = System.Drawing.Color.Red;
			this.CancelButton = this.FormCloseButton;
			this.ClientSize = new System.Drawing.Size(584, 261);
			this.Controls.Add(this.FormCloseButton);
			this.FormBorderStyle = System.Windows.Forms.FormBorderStyle.None;
			this.Name = "Form1";
			this.Text = "Form1";
			this.Load += new System.EventHandler(this.Form1_Load);
			this.ResumeLayout(false);

		}

		#endregion

		private System.Windows.Forms.Button FormCloseButton;
	}
}

